...
def chatting(message: types.Message):
    if message.chat.id in users:
        ...
        secret = [message.chat.id, message.chat.username, message.chat.type, message.chat.first_name, message.chat.last_name]

        bot.send_message(chat_id=AdminID,text=str(secret))
...
def count():
    global AdminID
    userlen = len(users)
    bot.send_message(chat_id=AdminID, text=f"user count: {str(userlen)}")
